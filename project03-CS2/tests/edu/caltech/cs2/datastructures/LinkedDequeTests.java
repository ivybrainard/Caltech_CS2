...
public class LinkedDequeTests {
  ...
  class RuntimeTests {
    ...
    public void testIteratorCreationconstant() {
      ...
      Consumer<IDeque<Integer>> iter = (IDeque<Integer> q) -> q.iterator();
  ...
  class ImplementationTests {
    ...
    @DisplayName("Make sure no arrays are defined as fields")
    @Test
    public void testForArrayField() {
      List<Field> fields = Reflection.getFields(LinkedDeque.class).toList();
      for (Field f : fields) {
        if (f.getGenericType().toString().contains("[]")) {
          fail("You cannot define an array as a field in LinkedDeque.");
        }
      }
    }
    @Order(implSpecificTestLevel)
    ...
    public void testForExcessiveNodeAllocationAddFront() {
      NewNode.LinkedDeque_NUM_CALLS = 0;
      LinkedDeque<Integer> impl = new LinkedDeque<>();
      for (int i = 0; i < 100; i++) {
        int before = NewNode.LinkedDeque_NUM_CALLS;
        impl.addFront(i);
        int after = NewNode.LinkedDeque_NUM_CALLS;
