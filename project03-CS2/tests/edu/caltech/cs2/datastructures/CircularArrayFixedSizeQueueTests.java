...
public class CircularArrayFixedSizeQueueTests {
  ...
  class RuntimeTests {
    ...
    @DisplayName("iterator() takes constant time")
    ...
    public void testIteratorCreationconstant() {
      ...
      Consumer<IFixedSizeQueue<Integer>> iter = (IFixedSizeQueue<Integer> q) -> q.iterator();

      RuntimeInstrumentation.assertAtMost("iterator", RuntimeInstrumentation.ComplexityType.CONSTANT, provide, iter, 8);
    }
    @Order(complexityTestLevel)
  ...
  class ImplementationTests {
    ...
    @DisplayName("Make sure exactly one generic E[] is defined")
    @Test
    public void testForArrayField() {
      List<Field> fields = Reflection.getFields(CircularArrayFixedSizeQueue.class).toList();
      int num = 0;
      for (Field f : fields) {
        String type = f.getGenericType().toString();
        if (type.contains("[")) {
          if (!type.equals("E[]")) {
            fail("You cannot define a non-generic array as a field in this class.");
          }
          num++;
        }
      }
      if (num != 1) {
        fail("You must define exactly one generic array as a field in this class.");
      }
    }
    ...
    public void testToString(String inputs) {
      ...
      IFixedSizeQueue<Object> me = new CircularArrayFixedSizeQueue<>(10);
