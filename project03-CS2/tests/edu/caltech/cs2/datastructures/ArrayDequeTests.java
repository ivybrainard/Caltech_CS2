...
public class ArrayDequeTests {
  ...
  class RuntimeTests {
    ...
    public void testIteratorCreationconstant() {
      Function<Integer, IDeque<Integer>> provide = (Integer numElements) -> {
        Constructor c = Reflection.getConstructor(ArrayDeque.class, int.class);
  ...
  class ImplementationTests {
    ...
    @DisplayName("Make sure a node class is not defined")
    @Test
    public void testForNodeClass() {
        Class[] classes = ArrayDeque.class.getDeclaredClasses();
        boolean found = false;
        ...
        if (found) {
          fail("There an inner class defined in ArrayDeque other than the Iterator.  This is not allowed.");
        }
    }
